 ...
 */
public class LineBaseClientHandler extends SimpleChannelInboundHandler<ByteBuf> {//处理某一特定类型的数据
...
//        ctx.writeAndFlush(Unpooled.copiedBuffer("Hello Netty", CharsetUtil.UTF_8));
        ...
        //测试粘包和半包
        ByteBuf msg = null;
        String line = "xiaowang,xiaoli,xiaohei,zhangsan,wangwu,lisi,zhaoba"
                +System.getProperty("line.separator");//这就是个换行符，在Linux和Win上不同，所以用这个JDK自己适配

        for(int i=0;i<100;i++){
            msg = Unpooled.buffer(line.length());
            msg.writeBytes(line.getBytes());
